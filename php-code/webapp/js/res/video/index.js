layui.use([ 'table', 'element', 'form' ], function() {
	...
	var form = layui.form;
	var url = window.siteUrl + '/res/res_video/list_data/wait_publish'
	...
	element.on('tab(demo)', function(data) {
		if ($(this).attr('id') == 'waitPublishTab') {
			url = window.siteUrl + '/res/res_video/list_data/wait_publish';
		} else if ($(this).attr('id') == 'publishTab') {
			url = window.siteUrl + '/res/res_video/list_data/publish';
		} else if ($(this).attr('id') == 'soldOutTab') {
			url = window.siteUrl + '/res/res_video/list_data/sold_out';
		}
		table.reload("table", {
			url : url
		})
	...
	table.on('toolbar(table)', function(obj) {
		...
		case 'delete':
			batch(checkStatus.data, window.siteUrl
					+ '/res/res_video/delete_batch');
			break;
		case 'publish':
			batch(checkStatus.data, window.siteUrl
					+ '/res/res_video/publish_batch');
			break;
		case 'sold_out':
			batch(checkStatus.data, window.siteUrl
					+ '/res/res_video/sold_out_batch');
			break;
		}
		;
	});
	...
	function showEdit(id) {
		...
		window.location.href = window.siteUrl
				+ '/webapp/pages/res/video/edit.html';
	}

	// 跳转添加页面
	function add() {
		window.location.href = window.siteUrl
				+ '/webapp/pages/res/video/add.html';
	}
	// 批量删除
...
/*
 * layui.use(['form', 'layedit', 'laydate'], function() { var form = layui.form ,
 * layer = layui.layer , layedit = layui.layedit , laydate = layui.laydate; });
 * var img_path ="" layui.use('upload', function(){ var $ = layui.jquery ,upload =
 * layui.upload;
 * 
 * //普通图片上传 var uploadInst = upload.render({ elem: '#test1' ,url: window.siteUrl +
 * '/upload_oss/testImg/img' ,before: function(obj){ //预读本地文件示例，不支持ie8
 * window['objs'] = obj; } ,done: function(res){ //如果上传失败 if(res.code == 0){
 * return layer.error(res.msg); }else{ objs.preview(function(index, file,
 * result){ $('#demo1').attr('src', result); //图片链接（base64） });
 * layer.msg(res.msg); } } ,error: function(){ //演示失败状态，并实现重传 var demoText =
 * $('#demoText'); demoText.html('<span style="color: #FF5722;">上传失败</span> <a
 * class="layui-btn layui-btn-mini demo-reload">重试</a>');
 * demoText.find('.demo-reload').on('click', function(){ uploadInst.upload();
 * }); } }); });
 * 
 * $(document).ready(function(){ $.ajax({ type: "GET", url: window.siteUrl +
 * '/upload_oss/Signatureurl', dataType: "json", success: function (data, msg) {
 * decodeUnicode(data.data); layer.msg(data.data)
