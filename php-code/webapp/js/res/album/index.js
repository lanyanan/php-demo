layui.use([ 'table', 'element', 'form' ], function() {
	...
	var form = layui.form;
	var url = window.siteUrl + '/res/res_album/list_data/wait_publish';
	...
	element.on('tab(demo)', function(data) {
		if ($(this).attr('id') == 'waitPublishTab') {
			url = window.siteUrl + '/res/res_album/list_data/wait_publish';
		} else if ($(this).attr('id') == 'publishTab') {
			url = window.siteUrl + '/res/res_album/list_data/publish';
		} else if ($(this).attr('id') == 'soldOutTab') {
			url = window.siteUrl + '/res/res_album/list_data/sold_out';
		}
		table.reload("table", {
			url : url
		})
	...
	table.on('toolbar(table)', function(obj) {
		...
		case 'delete':
			batch(checkStatus.data, window.siteUrl
					+ '/res/res_album/delete_batch');
			break;
		case 'publish':
			batch(checkStatus.data, window.siteUrl
					+ '/res/res_album/publish_batch');
			break;
		case 'sold_out':
			batch(checkStatus.data, window.siteUrl
					+ '/res/res_album/sold_out_batch');
			break;
		}
		;
	});
	...
	function showEdit(id) {
		...
		window.location.href = window.siteUrl
				+ '/webapp/pages/res/album/edit.html';
	}

	// 跳转添加页面
	function add() {
		window.location.href = window.siteUrl
				+ '/webapp/pages/res/album/add.html';
	}
	// 批量删除
